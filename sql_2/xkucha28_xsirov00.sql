...
INSERT INTO "message" ("title", "text", "valid_from", "valid_to", "school_id", "faculty_id")
VALUES ('Uvítací zpráva', 'vítejte na naší škole!', TO_TIMESTAMP('2023-03-23 10:00:00', 'YYYY-MM-DD HH24:MI:SS'), NULL, 2, NULL);
INSERT INTO "message" ("title", "text", "valid_from", "valid_to", "school_id", "faculty_id")
VALUES ('Přijímací skoušky', 'Přijímací skoušky se uskuteční 22.05.2023', TO_TIMESTAMP('2023-03-29 10:00:00', 'YYYY-MM-DD HH24:MI:SS'), NULL, 2, NULL);
...
-- all faculties and their info with atleast one message
SELECT f.* FROM "faculty" f INNER JOIN "message" m ON f."id" = m."faculty_id" WHERE EXISTS (SELECT m."faculty_id" FROM "message");

-- count of messages of every school
SELECT "school_id", COUNT(*) AS "message_count" FROM "message" GROUP BY "school_id";

-- count of messages of every faculty
SELECT "faculty_id", COUNT(*) AS "message_count" FROM "message" GROUP BY "faculty_id";

-- select all changes taht contain the word "chyba" followed and preceeded by anything
SELECT * FROM "message" m JOIN "edit" e ON m."id" = e."message_id" WHERE e."changes" LIKE '%chyba%';

-- select all messages from schools with description longer than 25 charracters and school name and description
SELECT m.*, s."name", s."description" FROM "message" m JOIN "school" s ON m."school_id" = s."id" WHERE LENGTH( s."description" ) > 30;

-- title and text, changes made by admins with permission level 0
SELECT m."title", m."text", e."changes" FROM "message" m JOIN "edit" e ON m."id" = e."message_id" JOIN "admin" a ON e."admin_id" = a."id" WHERE a."permission_level" = 0;

-- select all ids, names and emails of users from schools that published more than 1 message
SELECT * FROM "school_user" su JOIN "user" u ON su."user_id" = u."id" WHERE su."school_id" IN (SELECT "school_id" FROM "message" GROUP BY "school_id" HAVING COUNT(*) > 1);
